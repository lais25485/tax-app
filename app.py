...
# --- 4. 基礎データ定義（提供資料に基づく2026年最新値） ---
YEAR = 2026
WALL_DETAILS = {
    "106万：社会保険（51人以上の企業・撤廃時期未定）": 1060000,
    "130万：自分の社保を免除（一般）": 1300000,
    "136万：親や配偶者の税金を増やさない": 1360000, # 令和8年度大綱：所得62万反映
    "178万：自分の所得税を0円にする（令和8年特例）": 1780000 # 令和8年度大綱：特例加算反映
}
...
# --- 6. アプリ設定（スマホ版チラつき防止） ---
st.set_page_config(
    page_title="2026年 年収の壁シミュレーター",
    layout="centered",
    initial_sidebar_state="collapsed"
)

# --- 7. 外部ツール連携 (Google Search Console) ---
components.html(
    """<meta name="google-site-verification" content="j6no7A2pU6Xtd7DETM3KgDi7O8H" />""",
    height=0,
)
...
if user_resp and user_resp.user:
    ...
    if st.sidebar.button("💾 データをクラウドに保存", type="primary", use_container_width=True):
        ...
        st.sidebar.success("クラウドに保存しました！")
    ...
    try:
        ...
        st.sidebar.markdown(
            f'<a href="{auth_res.url}" target="_top" style="text-decoration:none;"><div style="background-color:#FF4B4B;color:white;padding:0.6rem;border-radius:8px;text-align:center;font-weight:600;">🌐 Googleでログイン</div></a>',
...
if st.session_state.step == "diagnosis":
    ...
    with st.form("survey"):
        ...
        with col_y:
            sel_year = st.selectbox("年", range(1960, YEAR + 1), index=YEAR-1960-21)
        with col_m:
            sel_month = st.selectbox("月", range(1, 13), index=3)
        with col_d:
            sel_day = st.selectbox("日", range(1, 32), index=0)
...
# --- 10. B. 計算モード ---
    ...
    with tab1:
        ...
        with c1:
            ...
            # 均等割（確定的な目安）
            if est > jumin_limit_base:
                st.error("❌ **均等割:** 発生見込み")
            else:
                st.success("✅ **均等割:** 非課税目安")
            # 所得割（不確定・目安）
            if est > jumin_limit_base + 50000:
                st.error("❌ **所得割:** 発生の可能性高")
            else:
                st.warning("⚠️ **所得割:** 自治体により変動")

            st.subheader("⚖️ 所得税 (確定)")
            if est > 1780000:
                st.error("❌ **所得税:** 発生")
            else:
                st.success("✅ **所得税:** 0円 (令和8年特例)")
            
        with c2:
            st.subheader("🏥 社会保険")
            if est >= st.session_state.shaho_limit:
                st.error("❌ **社会保険:** 扶養外")
            else:
                st.success("✅ **社会保険:** 扶養内")
            st.caption("※106万円の壁は最低賃金の動向により撤廃時期未定")

            st.subheader("👨‍👩‍👧 扶養の壁 (確定)")
            if est > 1360000:
                st.error("❌ **サポーター:** 増税リスク")
            else:
                st.success("✅ **サポーター:** 影響なし")
        
    with tab2:
        selected_key = st.selectbox("設定した上限額", list(WALL_DETAILS.keys()), index=list(WALL_DETAILS.keys()).index(st.session_state.target_key))
...
st.markdown(f"""
### 2026年（令和8年）の重要改正ポイント
提供資料に基づき、令和8年の確定申告における重要な壁を整理しました。

1. **所得税の壁（178万円）**: 
   令和8年度税制改正大綱により、所得税の非課税枠が特例的に**178万円**へ引き上げられました。これにより、学生や主婦の多くが所得税を気にせず働けるようになります。
2. **扶養の壁（136万円）**: 
   扶養親族の所得要件が62万円に緩和されたことで、給与収入**136万円**まで親や配偶者の税額控除が満額維持されます。
3. **社会保険の壁（106万/130万）**: 
   首相官邸資料によると、106万円の壁（賃金要件）は将来的に廃止される方針ですが、最低賃金の上昇を見極めた後の実施となるため、**現時点では撤廃時期は未定**です。
4. **住民税**: 
   自治体により異なりますが、均等割と所得割があり、改正に伴い非課税限度額も底上げされています。

---
*※本ツールは令和8年度税制改正大綱、令和7年度大綱、首相官邸資料に基づき作成されています。*
""")
